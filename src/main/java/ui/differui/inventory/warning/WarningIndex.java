...
public class WarningIndex extends MyPanel implements ActionListener{
	...
	MyComboBox comboBox, comboBox_sort;
	...
	public static ArrayList<BillListItem> commoList;
 	...
 	public WarningIndex(){
		...
		String[] comboBoxStr2 = {"请选择警报类单据类别", "报溢单", "报损单", "报警单"};
		comboBox_sort = new MyComboBox(83, 60 + y, 225, 25,comboBoxStr2);
		...
		jsp.setBounds(50 + 33, 95 + y, 225, 425 + 31);
		jsp.setVisible(true);
		this.add(jsp);
	...
	public void actionPerformed(ActionEvent events) {
		...
		if(events.getSource() == button_finish){
			...
			if(addCommoID.size() == 0){
				...
				wf.setVisible(true);
			}else if(comboBox_sort.getSelectedIndex() == 0 && addCommoID.size() != 0){
				WarningFrame wf = new WarningFrame("请选择一种警报类单据类型!");
				wf.setVisible(true);
			}else{
	
				this.setVisible(false);
				Frame_Inventory.visibleFalse(7);
				...
				for(int i = 0; i < addCommoID.size(); i++){
					...
					WarningFinish.alarmType = comboBox_sort.getSelectedIndex();
					
					WarningFinish.showText.doClick();
				}
			}
		}	
	}
