...
public class SaleBackFinal extends JLabel implements ActionListener{
	...
	public void actionPerformed(ActionEvent events){
		...
		if(events.getSource() == button_finish){
			...
			controller.submit(new saleAddVO(null, 0, 0, SaleBackFirst.note));
					
			WarningFrame wf = new WarningFrame("销售退货单添加成功！");
			wf.setVisible(true);
		...
		if(events.getSource() == button_output){
			...
			String file = String.valueOf(fsv.getHomeDirectory())+"/销售退货单"+ ID +".txt";		
	...
	public String showText(){
		...
		for(int i = 0; i < 35; i++)
			text = text + blank4;
		text = text + "客户名称：" + SaleBackFirst.ClientName + "\n";
